...
import com.example.boardinghousemanage.database.PhieuThu
...
class GeneralViewModelFactory(
    ...
    private val databasePhieuThu: PhieuThuDao,
    private val application: Application
) : ViewModelProvider.Factory {
    @Suppress("unchecked_cast")
    override fun <T : ViewModel?> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(GeneralViewModel::class.java)) {
            return GeneralViewModel(MaPhong, databasePhong, databaseThamSo, databasePhieuThu, application) as T
        }
